...
for (variable in mutations) {
  for (age_group in unique_age_groups) {
    ...
    if (sum(!is.na(filtered_df[[variable]])) > 0) {
      ...
      status_age <- filtered_df[[endpoint_stat]]
      
      if (sum(!is.na(x_age)) > 1) {
        model_age <- coxph(Surv(y_age, status_age) ~ x_age)
        model_summary_age <- summary(model_age)
        
        p_value_age <- coef(summary(model_age))[, "Pr(>|z|)"]
        pvalue_mutations <- c(pvalue_mutations, p_value_age)
      }
    }
  }
}
...
for (variable in mutations) {
  ...
  headers <- c("Age Group", "HR", "Lower CI", "Upper CI", "p-value", "p_adj")
  ...
  for (age_group in unique_age_groups) {
    ...
    if (sum(!is.na(filtered_df[[variable]])) > 0) {
      ...
      if (sum(!is.na(x_age)) > 1) {
        ...
        # Calculate the p_adj for this row
